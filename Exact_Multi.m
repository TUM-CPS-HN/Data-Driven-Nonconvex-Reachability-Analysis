function Z = Exact_multi(Z1,Z2,system_info,varargin)
...
for i=1:size(Z1.G,2)
    Z1A{i} = reshape(Z1.A(:,i), [], 1);
end

% Z1B = reshape(Z1.b, nx, nx+nu);
Z1B = reshape(Z1.b, [], 1);
...
% A
tempnewA=[];
if(~isempty(Z1A) && ~isempty(1))
    for i=1:length(Z1A)
        tempA = reshape(Z1A{i}, [], 1);
        tempnewA = [tempnewA tempA];
        if isequal(vecMultip,zeroVec)
                continue;
        end
    end
    newA =[tempnewA                              zeros(size(tempnewA,1),size(Z2.A,2));
           zeros(size(Z2.A,1),size(tempnewA,2))  Z2.A                               ];
end

% b
if(~isempty(Z1B) && ~isempty(1))
    newb = [reshape(Z1B, [], 1);
            Z2.b];
end
