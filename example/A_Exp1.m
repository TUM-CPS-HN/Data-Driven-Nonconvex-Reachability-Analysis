...
%Number of trajectories
steps1 = 15; % new data start point steps = 200 steps1 = 100
steps = 50;
...
G0=0.1*diag(ones(dim_x,1));

E0 = [2 1 0 0 0;   % x1 = α1^2 + α2
      1 2 0 0 0;   % x2 = α1 + α2^2
      0 0 2 1 0;   % x3 = α3^2 + α4
      0 0 1 2 1;   % x4 = α3 + α4^2
      0 0 0 1 2];  % x5 = α4^2 + α5^2
X0 = conPolyZono(ones(dim_x,1),G0,E0);
...
% init sets for loop
X_model{1} = X0; X_data{1} = X0; X_exact{1} = X0; X_all{1} = X0;X_exact2{1} = X0;
polyM1 = conPolyZono(M1);
redOp=800;
for i=1:totalsteps
    ...
    P2 = cartProd(X_exact2{i},polyU);
    X_exact2{i+1,1} = exactPlusWConst(Exact_multi_initi(polyM1,P2,system_info),polyW);
    X_exact2{i+1,1}=reduce(X_exact2{i+1,1},'girard',redOp);
...
for plotRun=1:length(projectedDims)
    ...
    for iSet=2:numberofplots
        handleAll=   plot(X_exact2{iSet},projectedDims{plotRun},'k');
    end
    ...
    legend([handleX0,handleModel,handleExact,handleAll],...
        {'Initial Set','Model Based','$\hat{\mathcal{R}}$ Algorithm 1', '$\hat{\mathcal{R}}_1$ Initial Reachability'}, ...
